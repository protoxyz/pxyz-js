...
export function PhoneVerification({ mode }: PhoneVerificationProps) {
  ...
  const onAttemptPhoneVerification = async (
  ...
  const loading =
    isAttemptingSignUpVerification || isAttemptingSignInFirstFactor;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-md text-zinc-400">
        Enter the 6-digit verification code we just texted to{' '}
        <b className="text-zinc-900">{phone}</b>.
      </div>

      <form
        onSubmit={loading ? undefined : onAttemptPhoneVerification}
