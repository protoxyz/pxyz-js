...
export function UserHeader() {
    ...
    const brandElement = useMemo(() => {
        if (theme?.brandLogo) {
            return (
                <Image
                    className={clsx("hidden h-8 w-auto lg:block", theme?.headerBrandLogoFill)}
                    src={theme?.brandLogo}
                    alt={theme?.brandName ?? ""}
                />
            );
        } else {
            return <h1 className={clsx("text-2xl font-bold", theme?.headerBrandNameColor)}>{theme?.brandName}</h1>;
        }
    }, [instance, theme]);
