  ...
  const onClick = () => {
    switch (mode) {
      case OAuthMode.signIn:
        {
          createSignInAttempt({
            strategy: AuthVerificationStrategy.oauth,
  ...
  const isLoading = useMemo(() => {
    switch (mode) {
      case OAuthMode.signIn: {
        return (
          isCreatingOAuthSignInAttempt &&
          providerKey === oauthSignInAttemptProvider
        );
      }
      case OAuthMode.signUp: {
        return (
          isCreatingOAuthSignUpAttempt &&
          providerKey === oauthSignUpAttemptProvider
        );
      }
      case OAuthMode.connection: {
      ...
      >
        ...
        {!isLoading && (
          <>
            ...
            {type === ThemeSocialType.icon && !Icon && (
...
export function SocialButtons({
  ...
  return (
    <div className="mt-6">
      {hasNonSocialSignIn && placement === ThemeSocialPlacement.bottom && (
        <Divider />
      )}
      ...
      <div
        className={clsx(
          'grid gap-3',
          gridColsClass,
          ThemeSocialPlacement.top ? 'mb-6' : 'mt-6',
        )}
      >
      ...
      {hasNonSocialSignIn && placement === ThemeSocialPlacement.top && (
        <Divider />
      )}
    </div>
  );
