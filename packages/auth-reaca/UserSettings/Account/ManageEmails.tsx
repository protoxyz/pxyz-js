...
import { ResponseStatus, EmailAddress } from '@protoxyz/types';
...
export function ManageEmailAddresses() {
  ...
  const [verifyingEmailId, setVerifyingEmailId] = useState<
    string | undefined
  >();
  ...
  async function deleteEmail(emailId: string) {
    ...
    const result = await client.auth.emailAddresses.delete({
      path: { emailId },
    });
  ...
  async function verify(
    ...
    const result = await client.auth.emailAddresses.verify({
      path: { emailId },
      body: { code },
    });
  ...
  async function resendVerification(emailId: string) {
    ...
    const result = await client.auth.emailAddresses.resendVerification({
      path: { emailId },
    });
  ...
  async function setPrimary(emailId: string) {
    ...
    const result = await client.auth.emailAddresses.setPrimary({
      path: { emailId },
    });
            ...
            <div key={email.id}>
              ...
              >
                <div className="flex items-center gap-x-2">
                  ...
                  {email.id !== user?.primaryEmailId &&
                    (email.verifiedAt ||
                      instance?.emailVerificationRequired === false) && (
                      <>
                        <Button
              ...
              {!email.verifiedAt && (
                <div className="p-5">
                  ...
                  <div className={clsx('mt-1 text-xs leading-5 text-zinc-600')}>
                    ...
                    >
                      {resendingVerificationId === email.id && (
                        <Spinner size="xs" />
                      )}
