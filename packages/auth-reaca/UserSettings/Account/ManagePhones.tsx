...
export function ManagePhoneNumbers() {
  ...
  const [verifyingPhoneId, setVerifyingPhoneId] = useState<
    string | undefined
  >();
  ...
  async function verify(
    ...
    const result = await client.auth.phoneNumbers.verify({
      path: { phoneId },
      body: { code },
    });
  ...
  async function resendVerification(phoneId: string) {
    ...
    const result = await client.auth.phoneNumbers.resendVerification({
      path: { phoneId },
    });
  ...
  async function setPrimary(phoneId: string) {
    ...
    const result = await client.auth.phoneNumbers.setPrimary({
      path: { phoneId },
    });
            ...
            <div key={phone.id}>
              ...
              >
                <div className="flex items-center gap-x-2">
                  ...
                  {phone.id !== user?.primaryPhoneId &&
                    (phone.verifiedAt ||
                      instance?.phoneVerificationRequired === false) && (
                      <>
                        <Button
              ...
              {!phone.verifiedAt && (
                <div className="p-5">
                  ...
                  <div className={clsx('mt-1 text-xs leading-5 text-zinc-600')}>
                    ...
                    >
                      {resendingVerificationId === phone.id && (
                        <Spinner size="xs" />
                      )}
