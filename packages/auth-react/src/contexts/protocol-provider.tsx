...
export const ProtocolAuthProvider = ({
  ...
  const [clientState, setClientState] =
    React.useState<ProtocolAuthClientContextState>({
      ...
      setSignIn: (signIn: SignInAttempt | null) => {
      ...
      setSignUp: (signUp: SignUpAttempt | null) => {
        setClientState((state) => ({
          ...state,
          signUp,
        }));
...
const getFirstFactorStrategies = (
  ...
  const strategies: AllowedFirstFactorStrategy[] = [];
...
const getSecondFactorStrategies = (
  ...
  const strategies: AllowedSecondFactorStrategy[] = [];
...
const getInitialFirstFactorStrategy = (
  strategies: AllowedFirstFactorStrategy[] | null | undefined,
): AllowedFirstFactorStrategy | null | undefined => {
...
const getInitialSecondFactorStrategy = (
  strategies: AllowedSecondFactorStrategy[] | null | undefined,
): AllowedSecondFactorStrategy | null | undefined => {
  if (!strategies) return null;
  if (strategies.length > 0) return strategies[0];
...
const getAccessTokenFromLocalStorage = (tenant: Tenant | null |undefined) => {
  if (
    isBrowser() &&
