...
export function SignInVerifyFirstFactorRoute() {
    ...
    const resendCode = async () => {
        ...
        setCodeResent(false);
        const resendResponse = await protocol.auth.signInAttempts.prepareFirstFactor({
            path: {
                id: signIn.id,
            },
            body: {
...
export function SignInVerifyFirstFactorForm({ setError }: { setError: (error: string) => void }) {
    const { setRoute } = useProtocolAuthSignInFlow();
    const { protocol } = useProtocolAuth();
    const { signIn, setSignIn } = useProtocolAuthClient();
    ...
    const [ref0, ref1, ref2, ref3, ref4, ref5] = [useRef(), useRef(), useRef(), useRef(), useRef(), useRef()];
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        const code = [values.code_0, values.code_1, values.code_2, values.code_3, values.code_4, values.code_5].join(
            "",
        );

        const signInResponse = await protocol.auth.signInAttempts.attemptFirstFactor({
            path: {
                id: signIn.id,
            },
            body: {
