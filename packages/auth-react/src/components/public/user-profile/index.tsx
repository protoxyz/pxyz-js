...
import { UserProfileFlowRoute, useProtocolAuthUserProfileFlow } from "../../../contexts/flow-context";
...
import { LaptopIcon, LinkIcon, MailIcon, PhoneIcon, ShieldCheckIcon, ShieldIcon, User, UserIcon } from "lucide-react";
        ...
        <CardWrapper component={component} className={appearance?.elements?.cardWrapper}>
            <Card className={appearance?.elements?.card}>
                <CardHeader className={appearance?.elements?.cardHeader}>
                    <CardTitle className={appearance?.elements?.cardHeaderTitle}>{userDisplayName(user)}</CardTitle>
                ...
                <CardContent className={appearance?.elements?.cardContent}>
                    <div className="flex flex-col space-y-8 lg:flex-row lg:space-x-8 lg:space-y-0">
                        ...
                        <div className=" flex-1  ">
                            {route === UserProfileFlowRoute["userProfile:settings"] && <UserSettingsRoute />}
                            {route === UserProfileFlowRoute["userProfile:security"] && <SecurityRoute />}
                            {route === UserProfileFlowRoute["userProfile:sessions"] && <SessionsRoute />}
                            {route === UserProfileFlowRoute["userProfile:emails"] && <UserEmailsRoute />}
                            {route === UserProfileFlowRoute["userProfile:phones"] && <UserPhonesRoute />}
                            {route === UserProfileFlowRoute["userProfile:connections"] && <UserConnectionsRoute />}
                        </div>
                    </div>
                </CardContent>
            </Card>
