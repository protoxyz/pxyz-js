...
function FooterLink({ text, href }: { text: string; href: string }) {
    return (
        <a target="_blank" rel="noreferrer" href={href} className="pxyz-auth-footer-link">
            {text}
        </a>
    );
}

export function CardFooterLinks({ component }: { component: AuthComponentType }) {
    ...
    const footerLinks = useMemo(() => {
        ...
        return links;
    }, [appearance?.layout?.helpUrl, appearance?.layout?.privacyPolicyUrl, appearance?.layout?.tosUrl]);

    if (!footerLinks.length || appearance?.layout?.showCopyrightAndTerms === false) return null;
    ...
    return (
        <div className="pxyz-auth-card-footer-links flex justify-between px-2 py-5 text-xs">
            ...
            <div className="pxyz-auth-footer-links text-muted-foreground flex items-center gap-3">{footerLinks}</div>
        </div>
    );
}
