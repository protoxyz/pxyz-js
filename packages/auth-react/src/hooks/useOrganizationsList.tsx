...
export function organizationsListCacheKey({ cursor, perPage, userId }: PaginatedArgs & { userId?: string }) {
...
export const useProtocolAuthOrganizationsList = ({ cursor = null, perPage = 10 }: PaginatedArgs) => {
    ...
    const { data, error, isLoading, mutate } = useSWR<ListOrganizations200Response>(
        cacheKey,
        () =>
    ...
    const deleteOrganization = useCallback(
        async ({ afterDeleteUri, onDelete }: { afterDeleteUri?: string; onDelete?: () => void }) => {
            setDeletingId(orgId);
