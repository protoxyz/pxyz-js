...
export function sessionsListCacheKey({ cursor, perPage, userId }: PaginatedArgs & { userId?: string }) {
    return [ListSessionsPath, userId, cursor, perPage].filter(Boolean).join(":");
}

export const useProtocolAuthSessionsList = ({ cursor = null, perPage = 10 }: PaginatedArgs) => {
    ...
    const { data, error, isLoading, mutate } = useSWR<ListSessions200Response>(cacheKey, () =>
