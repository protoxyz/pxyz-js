...
export function connectionsListCacheKey({ cursor, perPage, userId }: PaginatedArgs & { userId?: string }) {
    return [ListSocialConnectionsPath, userId, cursor, perPage].filter(Boolean).join(":");
}

export const useProtocolAuthConnectionsList = ({ cursor = null, perPage = 10 }: PaginatedArgs) => {
    ...
    const { data, error, isLoading, mutate } = useSWR<ListSocialConnections200Response>(
        cacheKey,
        () =>
            ...
            }),
        {
            fallbackData: {
                meta: {
                    ...
                    next: userConnections.length > 0 ? userConnections[userConnections.length - 1].id : null,
                    numPages: 1,
                    perPage: 10,
    ...
    const createConnection = useCallback(async (body: CreateSocialConnectionsOptions["body"]) => {
        setIsCreating(true);
