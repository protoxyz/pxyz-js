...
import { CreatePhoneNumberOptions, ListPhoneNumbers200Response, ListPhoneNumbersPath } from "@protoxyz/core";
import { useCallback, useState } from "react";

export function phonesListCacheKey({ cursor, perPage, userId }: PaginatedArgs & { userId?: string }) {
    return [ListPhoneNumbersPath, userId, cursor, perPage].filter(Boolean).join(":");
}

export const useProtocolAuthPhonesList = ({ cursor = null, perPage = 10 }: PaginatedArgs) => {
    ...
    const { data, error, isLoading, mutate } = useSWR<ListPhoneNumbers200Response>(
        cacheKey,
        () =>
            ...
            }),
        {
            fallbackData: {
                meta: {
                    ...
                    next: userPhones.length > 0 ? userPhones[userPhones.length - 1].id : null,
                    numPages: 1,
                    perPage: 10,
    ...
    const createPhone = useCallback(async ({ phone }: CreatePhoneNumberOptions["body"]) => {
        setIsCreating(true);
