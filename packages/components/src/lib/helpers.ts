...
export function luminanceFromRGB(r: number, g: number, b: number) {
  ...
  return 21.26 * (R as number) + 71.52 * (G as number) + 7.22 * (B as number);
}

export function luminanceFromHex(hex: string) {
  const { r, g, b } = hexToRGB(hex);

  return luminanceFromRGB(parseInt(r), parseInt(g), parseInt(b));
}
...
export function lightnessFromHSLum(H: number, S: number, Lum: number) {
  const vals = {} as any;
  for (let L = 99; L >= 0; L--) {
    const asdf = HSLtoRGB(H, S, L);
    vals[L] = Math.abs(Lum - luminanceFromRGB(asdf.r, asdf.g, asdf.b));
  }
...
export function hexToHSL(H: string) {
  ...
  if (delta === 0) h = 0;
  else if (cmax === numR) h = ((numG - numB) / delta) % 6;
  else if (cmax === numG) h = (numB - numR) / delta + 2;
  else h = (numR - numG) / delta + 4;
